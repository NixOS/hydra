...
impl State {
    ...
    async fn reject_builder_on_error<T>(
        &self,
...
fn spawn_fatal(fut: impl Future<Output = anyhow::Result<()>> + Send + 'static) {
...
impl RunnerService for Server {
        ...
        spawn_fatal(
            async move {
                state
                    ...
                    .await
                    .context("update_build_step failed")
            }
            .in_current_span(),
        );
    ...
    async fn fetch_requisites(
        ...
        let requisites = state
            ...
            .await
            .map_err(|e| tonic::Status::internal(format!("failed to query requisites: {e}")))?
            .into_iter()
