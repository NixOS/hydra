...
impl State {
    ...
    pub async fn succeed_step(
        ...
        // Write realisations for CA floating outputs to all remote stores.
        ...
        if let Some(drv_guard) = item.step_info.step.get_drv() {
            ...
            if has_ca_floating {
                let stores: Vec<RemoteStoreBackend> = self.remote_stores.read().clone();
                for (output_name, out_path) in &output.outputs {
                    ...
                    for store in &stores {
                        let res = match store {
                            RemoteStoreBackend::S3(s3) => s3
                                .write_realisation(realisation.clone())
                                .await
                                .map_err(|e| e.to_string()),
                            RemoteStoreBackend::Nix(nix_store) => nix_store
                                .register_drv_output(&realisation)
                                .map_err(|e| e.to_string()),
                        };
                        if let Err(e) = res {
                            tracing::warn!(
