...
#
# It cannot discover those jobs for itself: nothing in `hydra-eval-jobset`
# evaluates any more, so running one would mean scheduling and building an
...
subtest "a dry run reports the jobs" => sub {
    ...
    my ($res, $stdout, $stderr) = $ctx->capture_cmd(60,
        "hydra-eval-jobset", "--jobs", $jobsFile,
        $otherCtx->{"project"}->name, $other->name);
...
subtest "a dry run needs to be given the jobs" => sub {
    local $ENV{HYDRA_DRY_RUN} = "1";
    my ($res, $stdout, $stderr) = $ctx->capture_cmd(60,
        "hydra-eval-jobset", $otherCtx->{"project"}->name, $other->name);
