...
sub read_or_recover_password {
    ...
    # generate a new one.
    if (-s $self->{kanidm_dir} . "/${user}_password") {
...
sub recover_password {
    ...
    # The admin socket may not accept connections yet immediately after the
    # HTTPS server reports ready, so retry briefly rather than failing.
    my $json;
    my $json_text;
    for my $attempt (1 .. 10) {
        my @lines = do {
            open my $fh, '-|', 'kanidmd', '--config-path', $self->{kanidm_config},
                'scripting', 'recover-account', $user
                or die "Failed to run kanidmd command: $!";
            <$fh>;
        };
        $json_text = join "", @lines;
        $json = eval { decode_json($json_text) };
        $json = undef unless ref $json eq 'HASH';
        last if ($json->{status} // '') eq 'ok';

        die "Failed to recover password for $user: $json_text" if $attempt == 10;
        sleep 1;
    }

    # kanidm >= 1.10 reports the password under "output"; older versions
    # used a "password" key.
    my $password = $json->{output} // $json->{password};
    defined $password or die "kanidmd did not report a password for $user: $json_text";
    return $password;
}
