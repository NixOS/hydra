...
# Options:
#   max_db_connections: `maxDbConnections` for the daemon's config file.
#     Left unset for the daemon's default; set low to exercise pool
#     pressure.
sub new {
    my ($class, $ctx, %opts) = @_;
...
sub _spawn_ad_hoc {
    ...
    # Same data dir as start_queue_runner, so the daemon finds the
    # queue runner's build-logs and can stream them to the client.
    my $config_dir = $ENV{T2_HARNESS_TEMP_DIR} // $ctx->{central}{hydra_data};
    print $fh "hydraDataDir = \"$config_dir/data\"\n";
    print $fh "maxDbConnections = $self->{opts}{max_db_connections}\n"
        if defined $self->{opts}{max_db_connections};
    close $fh;
...
# Run several client commands concurrently, all pointed at hydra-ad-hoc.
# Returns one [rc, stdout, stderr] triple per command, in order. Each
# command gets the whole $timeout; a command still running at the
# deadline is killed and reported as failed.
sub run_cmds {
    ...
    while (time() < $deadline) {
        ...
        for my $i (0 .. $#jobs) {
            ...
            eval { $j->{h}->pump_nb };
            if ($@) {
                $results[$i] = [1, ${$j->{out}}, ${$j->{err}} . "\n[run_cmds: pump_nb error: $@]"];
                next;
            }
            if ($j->{h}->pumpable) {
                $pending++;
            ...
            $j->{h}->finish;
            my $rc = scalar $j->{h}->result;
            $results[$i] = [defined $rc ? $rc : 1, ${$j->{out}}, ${$j->{err}}];
        }
        last unless $pending;
    ...
    for my $i (0 .. $#jobs) {
        next if defined $results[$i];
        eval { $jobs[$i]{h}->kill_kill };
        $results[$i] = [1, ${$jobs[$i]{out}}, ${$jobs[$i]{err}} . "\n[run_cmds: timed out after ${timeout}s]"];
    }
    return @results;
}
