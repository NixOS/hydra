...
#
# Options:
#   rust_log: RUST_LOG value, default "hydra_queue_runner=debug,info".
...
sub start_queue_runner {
    ...
    my $qr_harness;
    {
        ...
        local $ENV{RUST_LOG} = $opts{rust_log} // "hydra_queue_runner=debug,info";
...
# Start a hydra-builder against an already-running queue runner, plus the
# nix daemon it builds through. Both are registered in $pg under the keys
# wait_for_builds looks for.
#
# Options:
#   rust_log: RUST_LOG value, default "hydra_builder=debug,info".
sub start_builder {
    my ($ctx, $grpc_addr, $pg, %opts) = @_;
    ...
    return $pg->spawn("builder",
        ...
        env => {
            ...
            RUST_LOG      => $opts{rust_log} // "hydra_builder=debug,info",
        },
    );
}
