    ...
    H: HandshakeDaemonStore + Clone + Send + Sync + 'static,
{
    ...
    /// lifetime of a daemon process. Connections still in flight when
    /// `shutdown` fires are dropped: whatever they were serving has
    /// gone away, so there is nobody left to answer.
