...
use crate::submit::{BuildRequest, SubmitBuild};
...
/// nix-daemon; build requests become Hydra `Builds` rows, with `S`
/// deciding which row (see [`SubmitBuild`]).
#[derive(Clone)]
pub struct HydraDaemonHandler<S> {
    store_dir: StoreDir,
    db: db::Database,
...
impl<S> std::fmt::Debug for HydraDaemonHandler<S> {
...
impl<S: SubmitBuild> HydraDaemonHandler<S> {
    pub fn new(
        store_dir: StoreDir,
        db: db::Database,
        upstream_socket: &str,
...
impl<S: SubmitBuild> HandshakeDaemonStore for HydraDaemonHandler<S> {
    type Store = Self;
...
impl<S: SubmitBuild> DaemonStore for HydraDaemonHandler<S> {
