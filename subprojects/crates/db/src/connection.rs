...
impl Transaction<'_> {
    ...
    /// Insert a kept, unfinished Build for a daemon-submitted derivation.
    ///
    /// The caller must register a waiter before committing, or the queue
    /// runner can finish the row before anyone is listening for it.
    #[tracing::instrument(skip(self), err)]
    pub async fn insert_daemon_build(
        &mut self,
        jobset_id: i32,
        nix_name: &str,
        drv_path: &str,
