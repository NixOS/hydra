...
# regenerated by `update-dbix`, which would drop these.
use Hydra::StorePath;
use Nix::StorePath;
...
sub resolved_terminal {
    ...
    # `resolveddrvpath` is stored bare, so it is a plain string rather than a
    # store path. Nothing constrains what is in the column, so a corrupt value
    # must not become an exception: anything that is not a store path simply
    # has no terminal, and the page falls through to the pending state.
