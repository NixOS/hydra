    ...
    W: AsyncWrite + Debug + Send + Unpin,
{
    async fn handshake(&mut self) -> DaemonResult<()> {
        ...
        let version = if client_version.minor() < PROTOCOL_VERSION.minor() {
            client_version
        } else {
            PROTOCOL_VERSION
        };
        ...
        // Exchange features (protocol >= 1.38): the client's set first,
        // then ours; both sides keep the intersection. Without this a
        // client cannot use content-addressed derivations through us.
        if version.minor() >= 38 {
            let client_features: FeatureSet = self.reader.read_value().await?;
            let ours = supported_features();
            self.writer.write_value(&ours).await?;
            self.writer.flush().await?;
            let features: FeatureSet = ours.intersection(&client_features).cloned().collect();
            self.reader.set_features(features.clone());
            self.writer.set_features(features);
        }
        ...
        S: DaemonStore + 's,
    {
        ...
        match request {
            ...
            // harmonia protocol extensions this daemon does not
            // implement. They are feature-gated and we advertise
            // neither, so a well-behaved client will not send them;
            // answer with a plain error rather than dropping the
            // connection if one does.
            op @ (SubmitOutput(_) | AddToStoreScanning(_)) => {
                return Err(RecoverableError {
                    can_recover: true,
                    source: DaemonError::unimplemented(op.operation()),
                });
            }
        }
        Ok(())
...
#[derive(Debug)]
pub(crate) struct DaemonServer<H> {
    handler: H,
    ...
    H: HandshakeDaemonStore + Clone + Send + Sync + 'static,
{
    ...
    /// down by systemd socket activation. Permissions of the socket
    /// file are theirs to manage.
    pub(crate) fn from_listener(handler: H, listener: UnixListener, store_dir: StoreDir) -> Self {
        Self {
            handler,
            listener,
            store_dir,
    ...
    /// Serve until the process is killed. The socket file, if we
    /// created it, is left behind: the next `bind` replaces it.
    pub(crate) async fn serve(&self) -> Result<(), std::io::Error> {
        loop {
