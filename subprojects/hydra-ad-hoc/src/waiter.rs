//! Async wakeup for Builds finished by the Hydra queue runner.
//!
...
const CHANNEL_STEP_STARTED: &str = "step_started";
...
/// A build step the queue runner announced on `step_started` or
/// `step_finished`. Fanned out to every handler that is showing a
/// build's logs; each filters for its own build id.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct StepEvent {
    pub(crate) build_id: BuildID,
    pub(crate) step_nr: i32,
    pub(crate) kind: StepEventKind,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum StepEventKind {
    Started,
    Finished,
}

/// Bounded because a slow log forwarder must not stall the listener;
/// a lagging receiver sees `Lagged` and re-syncs from the database.
const STEP_EVENT_CAPACITY: usize = 1024;
...
/// Registry of in-flight builds. Cloning shares the same backing
...
impl BuildWaiter {
    pub(crate) async fn start(db: &db::Database) -> Result<Self, db::Error> {
        ...
        let (steps, _) = broadcast::channel(STEP_EVENT_CAPACITY);
    ...
    /// Step announcements from now on. Subscribe *before* committing
    /// the build row, or its first steps can slip past.
    pub(crate) fn step_events(&self) -> broadcast::Receiver<StepEvent> {
    ...
    pub(crate) async fn forget(&self, build_id: BuildID) {
        ...
        loop {
            match stream.next().await {
                Some(Ok(notif)) => {
                    ...
                    dispatch(&notif, &waiters, &steps).await;
                }
                Some(Err(e)) => {
...
async fn sweep_already_finished(db: &db::Database, waiters: &WaiterMap) -> Result<(), db::Error> {
    ...
    let finished = crate::queries::finished_build_ids(conn.raw(), &registered).await?;
...
async fn dispatch(
    ...
    let step = match notif.channel() {
        CHANNEL_STEP_STARTED => build_logs::notify::parse_step_started_payload(payload)
            .map(|(b, s)| (b, s, StepEventKind::Started)),
        CHANNEL_STEP_FINISHED => build_logs::notify::parse_step_finished_payload(payload)
            .map(|(b, s)| (b, s, StepEventKind::Finished)),
        _ => None,
    };
