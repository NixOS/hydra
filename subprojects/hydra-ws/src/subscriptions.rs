...
fn finish_subscribers(subs: Vec<Subscriber>, msg: &HydraWsResponse) {
    for Subscriber { mut handle, tx, .. } in subs {
        let msg = msg.clone();
        tokio::spawn(async move {
            if tokio::time::timeout(DRAIN_TIMEOUT, &mut handle)
                .await
                .is_err()
            {
                tracing::warn!("Forwarder did not drain within {DRAIN_TIMEOUT:?}, aborting");
                handle.abort();
            }
            if tx.try_send(msg).is_err() {
                tracing::warn!("Failed to send completion notification, channel full");
            }
        });
    }
}
...
impl Subscriptions {
    ...
    fn finish_matching(
        &self,
        msg: &HydraWsResponse,
        pred: impl Fn(&(u64, Option<u64>)) -> bool,
    ) -> Vec<PathBuf> {
        ...
        for key in keys {
            if let Some((_, subs)) = self.inner.remove(&key) {
                if let Some(p) = subs.first().map(|s| s.path.clone()) {
                    paths.push(p);
                }
                finish_subscribers(subs, msg);
            }
        }
        paths
    }
...
mod tests {
    ...
    fn drained_handle() -> JoinHandle<()> {
        tokio::spawn(std::future::ready(()))
    }

    async fn recv(rx: &mut mpsc::Receiver<HydraWsResponse>) -> HydraWsResponse {
    ...
    async fn notify_step_finished_only_matches_exact_step() {
        ...
        let msg = recv(&mut rx_step).await;
        assert!(matches!(
            msg,
            HydraWsResponse::StepFinished {
                build_id: 1,
    ...
    async fn notify_build_finished_sends_messages_and_returns_paths() {
        ...
        for rx in [&mut rx1, &mut rx2] {
            ...
            assert!(matches!(
                msg,
                HydraWsResponse::BuildFinished { build_id: 1 }
            ));
        }
