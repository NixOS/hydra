...
mod tests {
    ...
    fn missing_config_falls_back_to_defaults() {
        ...
        let app = App::init(path.to_str().unwrap()).expect("missing file should use defaults");
        assert!(app.log_prefix.ends_with("build-logs"));
    }

    #[test]
    fn unreadable_config_is_an_error_not_a_silent_default() {
        let dir = tempfile::tempdir().unwrap();
        let err = App::init(dir.path().to_str().unwrap())
            .expect_err("a directory must not be treated as a missing config");
