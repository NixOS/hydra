...
/// Tell listeners an evaluation of this jobset has begun.
pub(crate) async fn notify_eval_started(
    conn: &mut db::Connection,
    trace: &str,
    jobset_id: i32,
) -> Result<(), sqlx::Error> {
    sqlx::query!(
        "SELECT pg_notify('eval_started', $1)",
        format!("{trace}\t{jobset_id}"),
    )
    .execute(conn.raw())
    .await?;
    Ok(())
...
pub(crate) async fn update_jobset_after_eval(
    ...
    sqlx::query!(
        "UPDATE jobsets SET startTime = null, lastCheckedTime = $2 WHERE id = $1",
        jobset_id,
        now,
    )
    .execute(&mut *tx)
    ...
    // `Some("")` is a success, and clears the error before it; `None` leaves
    // the error alone, for an outcome that says nothing about it.
    if let Some(error_msg) = error_msg {
        sqlx::query!(
            "UPDATE jobsets SET errorMsg = $1, fetchErrorMsg = null, \
               errorTime = CASE WHEN $1 = '' THEN NULL ELSE $2::bigint END \
             WHERE id = $3",
            error_msg,
            now,
            jobset_id,
...
pub(crate) async fn insert_eval_input(
    ...
    // Stored as a full path, which is what the schema's store path columns
    // hold; the fetcher reports bare names.
    let path = input.store_path.as_deref().map(|p| {
        if p.starts_with('/') {
            p.to_owned()
        } else {
            format!("{store_dir}/{p}")
...
pub(crate) async fn insert_build_outputs(
    ...
    let paths: Vec<Option<String>> = outputs.iter().map(|(_, p)| p.clone()).collect();
    // Unchecked, because the checked macro will not take an array with nulls
    // in it as a parameter, and a content-addressed output's path is null.
    sqlx::query(
