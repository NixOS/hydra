...
impl Evaluator {
    ...
    async fn should_evaluate_one_at_a_time(&self, jobset: &Jobset) -> bool {
        ...
        .await
        {
            Ok(v) => v,
            Err(e) => {
                tracing::error!("error checking one-at-a-time for {}: {e}", jobset.display());
                return false;
            }
        };
    ...
    async fn start_evals(&self, state: &mut State) -> anyhow::Result<()> {
        ...
        for jobset in state.jobsets.values() {
            if self.eval_one.is_some()
                || (jobset.evaluation_style.is_some() && self.should_evaluate(jobset, state).await)
            {
...
async fn update_db_after_eval(
    ...
    // Wrap in a transaction (as the C++ original did) so that a partial
    // failure cannot clear triggerTime but leave startTime set, which
    // would make the jobset appear permanently running.
...
/// Row type for the Jobsets query.  `lastcheckedtime` and `triggertime`
/// are INT4 in the Postgres schema; `sqlx` refuses to decode INT4 into
/// `i64`, so these must be `i32`.
#[derive(sqlx::FromRow)]
