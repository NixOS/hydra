    ...
    ref<Store> destStore, unsigned int & lastBuildId)
{
    ...
    createBuild = [&](Build::ptr build) {
        ...
        if (!localStore->isValidPath(build->drvPath)) {
            ...
            timer.finish(prom.queue_build_load_premature_gc);
            return;
        }
        ...
        try {
            ...
            timer.finish(prom.queue_build_load_cached_failure);
            return;
        }
        ...
        if (!step) {
            ...
            timer.finish(prom.queue_build_load_cached_success);
            return;
        }
        ...
        timer.finish(prom.queue_build_load_added);
    };
