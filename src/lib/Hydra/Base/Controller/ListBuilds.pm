...
sub all : Chained('get_builds') PathPart {
    ...
    $c->stash->{total} = $c->stash->{allBuilds}->search({finished => 1, fodchecker => 0})->count
        unless defined $c->stash->{total};

    $c->stash->{builds} = [ $c->stash->{allBuilds}->search(
        { finished => 1, fodchecker => 0 },
...
sub latest : Chained('get_builds') PathPart('latest') {
    ...
    my $latest = $c->stash->{allBuilds}->find(
        { finished => 1, buildstatus => 0, fodchecker => 0 }, { order_by => ["id DESC"], rows => 1 });
...
sub latest_for : Chained('get_builds') PathPart('latest-for') {
    ...
    my $latest = $c->stash->{allBuilds}->find(
        { finished => 1, buildstatus => 0, system => $system, fodchecker => 0 }, { order_by => ["id DESC"], rows => 1 });
...
sub latest_finished : Chained('get_builds') PathPart('latest-finished') {
    ...
    my $latest = $c->stash->{allBuilds}->find(
        { finished => 1, buildstatus => 0, fodchecker => 0 },
        { order_by => ["id DESC"], rows => 1, join => ["jobsetevalmembers"]
        , where => \
            "not exists (select 1 from jobsetevalmembers m2 join builds b2 on jobsetevalmembers.eval = m2.eval and m2.build = b2.id and b2.finished = 0 and b2.fodchecker = false)"
        });
