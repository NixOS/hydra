...
sub nix : Chained('get_builds') PathPart('channel/latest') CaptureArgs(0) {
    ...
    $c->stash->{channelBuilds} =
      ...
      ->search_literal("exists (select 1 from buildproducts where build = me.id and type = 'nix-build')")->search(
        {},
        {
...
sub latest : Chained('get_builds') PathPart('latest') {
    ...
    my $latest =
      $c->stash->{allBuilds}->find({ finished => 1, buildstatus => 0 }, { order_by => ["id DESC"], rows => 1 });
...
sub latest_for : Chained('get_builds') PathPart('latest-for') {
    ...
    my $latest = $c->stash->{allBuilds}
      ->find({ finished => 1, buildstatus => 0, system => $system }, { order_by => ["id DESC"], rows => 1 });
