...
sub jobsetOverview_ {
    ...
    return $jobsets->search({},
        ...
        , "+select" =>
          [ "(select count(*) from Builds as a where me.id = a.jobset_id and a.finished = 0 and a.isCurrent = 1 and a.fodchecker = false)"
          , "(select count(*) from Builds as a where me.id = a.jobset_id and a.finished = 1 and buildstatus <> 0 and a.isCurrent = 1 and a.fodchecker = false)"
          , "(select count(*) from Builds as a where me.id = a.jobset_id and a.finished = 1 and buildstatus = 0 and a.isCurrent = 1 and a.fodchecker = false)"
          , "(select count(*) from Builds as a where me.id = a.jobset_id and a.isCurrent = 1 and a.fodchecker = false)"
          ]
...
sub getEvalInfo {
    ...
    if (defined $nrSucceeded) {
        ...
        $nrScheduled = $eval->builds->search({finished => 0, fodchecker => 0})->count;
        $nrSucceeded = $eval->builds->search({finished => 1, buildStatus => 0, fodchecker => 0})->count;
        if ($nrScheduled == 0) {
