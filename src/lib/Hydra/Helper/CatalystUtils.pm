...
sub getLatestFinishedEval {
    ...
    my ($eval) = $jobset->jobsetevals->search(
        ...
        , where => \ "not exists (select 1 from JobsetEvalMembers m join Builds b on m.build = b.id where m.eval = me.id and b.finished = 0 and b.fodchecker = false)"
        });
    return $eval;
}
