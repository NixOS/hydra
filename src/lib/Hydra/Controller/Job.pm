...
sub overview : Chained('job') PathPart('') Args(0) {
    ...
    $c->stash->{lastBuilds} =
        [ $c->stash->{jobset}->builds->search({ job => $c->stash->{job}, finished => 1, fodchecker => 0 },
    ...
    my @constituents = $c->model('DB::Builds')->search(
        { aggregate => { -in => $c->stash->{jobset}->builds->search({ job => $c->stash->{job}, fodchecker => 0 }, { columns => ["id"], order_by => "id desc", rows => 15 })->as_query }, fodchecker => 0 },
...
sub get_builds : Chained('job') PathPart('') CaptureArgs(0) {
    my ($self, $c) = @_;
    $c->stash->{allBuilds} = $c->stash->{jobset}->builds->search({ job => $c->stash->{job}, fodchecker => 0 });
