...
sub findBuildStepByOutPath {
    my ($self, $c, $path) = @_;
    return $c->model('DB::BuildSteps')->search({ path => $path, busy => 0 },
        { join => ["buildstepoutputs"], order_by => [ "status", "stopTime" ], rows => 1 })->single;
}

sub findBuildStepByDrvPath {
    my ($self, $c, $drvPath) = @_;
    return $c->model('DB::BuildSteps')
      ->search({ drvpath => $drvPath, busy => 0 }, { order_by => [ "status", "stopTime" ], rows => 1 })->single;
}
...
sub build_GET {
    ...
    $self->status_ok($c, entity => $build);

    if (defined $c->stash->{eval}) {
        my ($eval2) =
          $c->stash->{eval}->jobset->jobsetevals->search({ hasnewbuilds => 1, id => { '<', $c->stash->{eval}->id } },
...
sub constituents_GET {
    ...
    $self->status_ok($c, entity => [ $build->constituents_->search({}, { order_by => ["job"] }) ]);
}
...
sub serveFile {
    ...
    my $res = run(cmd =>
          [ "nix", "--experimental-features", "nix-command", "ls-store", "--store", getStoreUri(), "--json", "$path" ]);
    ...
    if ($ls->{type} eq "directory" && substr($c->request->uri, -1) ne "/") {
        ...
        $c->stash->{'plain'} = {
            data => grab(
                cmd =>
                  [ "nix", "--experimental-features", "nix-command", "cat-store", "--store", getStoreUri(), "$path" ]
            )
        };
...
sub output : Chained('buildChain') PathPart Args(1) {
    ...
    $c->response->header('Content-Disposition',
        "attachment; filename=\"build-${\$build->id}-${\$outputName}.nar.bz2\"");
...
sub download_by_type : Chained('buildChain') PathPart('download-by-type') {
    ...
    (my $product) =
      $c->stash->{build}->buildproducts->search({ type => $type, subtype => $subtype }, { order_by => "productnr" });
...
sub nix : Chained('buildChain') PathPart('nix') CaptureArgs(0) {
    ...
    $c->stash->{channelBuilds} = $c->model('DB::Builds')->search(
        ...
        {
            join      => ["buildoutputs"],
            '+select' => [ 'buildoutputs.path', 'buildoutputs.name' ],
            '+as'     => [ 'outpath',           'outname' ]
        }
    );
}
...
sub keep : Chained('buildChain') PathPart Args(1) {
    ...
    $c->flash->{successMsg} = $keep ? "Build will be kept." : "Build will not be kept.";
...
"not exists (select 1 from jobsetevalmembers m2 join builds b2 on me.eval = m2.eval and m2.build = b2.id and b2.finished = 0)"
    ...
    $c->res->redirect(
        $c->uri_for($c->controller('JobsetEval')->action_for("view"), [ $eval->id ], @rest, $c->req->params));
}
...
sub reproduce : Chained('buildChain') PathPart('reproduce') Args(0) {
    ...
    $c->response->header('Content-Disposition',
        'attachment; filename="reproduce-build-' . $c->stash->{build}->id . '.sh"');
