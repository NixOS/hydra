...
sub view_GET {
    ...
    if (defined $compare) {
        if ($compare =~ /^\d+$/) {
            ...
            $eval2 =
              $c->stash->{jobset}
              ->jobsetevals->find({ hasnewbuilds => 1, timestamp => { '<=', $eval->timestamp - $t } },
        ...
        ($eval2) = $eval->jobset->jobsetevals->search({ hasnewbuilds => 1, id => { '<', $eval->id } },
...
sub nix : Chained('evalChain') PathPart('channel') CaptureArgs(0) {
    ...
    $c->stash->{channelBuilds} =
      $c->stash->{eval}
      ->builds->search_literal("exists (select 1 from buildproducts where build = build.id and type = 'nix-build')")
      ->search(
...
sub store_paths : Chained('evalChain') PathPart('store-paths') Args(0) {
    ...
    $self->status_ok($c, entity => [ uniq(sort map { $_->get_column('outpath') } @builds) ]);
}
...
sub all_builds : Chained('evalChain') PathPart('builds') Args(0) {
    ...
    $self->status_ok($c, entity => [@builds],);
}
