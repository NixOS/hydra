...
sub cancel : Chained('evalChain') PathPart('cancel') Args(0) {
    ...
    my $n = cancelBuilds($c->model('DB')->schema, $c->stash->{eval}->builds->search_rs({fodchecker => 0}));
...
sub restart {
    ...
    my $builds = $c->stash->{eval}->builds->search_rs({ finished => 1, buildstatus => $condition, fodchecker => 0 });
