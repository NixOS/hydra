...
sub begin :Private {
    ...
    if (scalar(@args) == 0 || $args[0] ne "static") {
        ...
        $c->stash->{nrQueuedBuilds} = $c->model('DB::Builds')->search({ finished => 0, fodchecker => 0 })->count();
    }
...
sub queue_summary :Local :Path('queue-summary') :Args(0) {
    ...
    $c->stash->{queued} = dbh($c)->selectall_arrayref(
        ...
        "where finished = 0 and fodchecker = false group by jobsets.project, jobsets.name order by queued desc",
        { Slice => {} });

    $c->stash->{systems} = dbh($c)->selectall_arrayref(
        "select system, count(*) as c from Builds where finished = 0 and fodchecker = false group by system order by c desc",
        { Slice => {} });
}
...
sub status_GET {
    ...
    $self->status_ok(
        $c,
        entity => [$c->model('DB::Builds')->search(
            { "buildsteps.busy" => { '!=', 0 }, fodchecker => 0 },
            { order_by => ["globalpriority DESC", "id"],
              join => "buildsteps",
