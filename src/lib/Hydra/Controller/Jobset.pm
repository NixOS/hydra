...
sub jobset_GET {
    ...
    $c->stash->{latestEval} =
      $c->stash->{jobset}->jobsetevals->search({ hasnewbuilds => 1 }, { rows => 1, order_by => ["id desc"] })->single;
...
sub jobset_PUT {
    ...
    if (defined $c->stash->{jobset}) {
        ...
        my $uri = $c->uri_for($self->action_for("jobset"), [ $c->stash->{project}->name, $c->stash->{jobset}->name ])
          . "#tabs-configuration";
        ...
        $c->model('DB')->schema->txn_do(
            sub {
                ...
                $jobset = $c->stash->{project}
                  ->jobsets->create({ name => ".tmp", nixexprinput => "", nixexprpath => "", emailoverride => "" });
...
sub jobs_tab : Chained('jobsetChain') PathPart('jobs-tab') Args(0) {
    ...
    my ($evals, $builds) =
      searchBuildsAndEvalsForJobset($c->stash->{jobset}, { job => { ilike => $filter }, ischannel => 0 }, 10000);
...
sub channels_tab : Chained('jobsetChain') PathPart('channels-tab') Args(0) {
    ...
    my ($evals, $builds) = searchBuildsAndEvalsForJobset($c->stash->{jobset}, { ischannel => 1 });
...
sub get_builds : Chained('jobsetChain') PathPart('') CaptureArgs(0) {
    ...
    $c->stash->{latestSucceeded} =
      $c->model('DB')->resultset('LatestSucceededForJobset')->search({}, { bind => [ $c->stash->{jobset}->id ] });
...
sub checkInputValue {
    ...
    error($c, "The value ‘$value’ of input ‘$name’ is not a Boolean (‘true’ or ‘false’).")
      if $type eq "boolean" && !($value eq "true" || $value eq "false");

    error($c,
            "The value ‘$value’ of input ‘$name’ does not specify a Hydra evaluation.  "
...
sub updateJobset {
    ...
    if ($type == 0) {
        foreach my $name (keys %{ $c->stash->{params}->{inputs} }) {
            ...
            badRequest($c, "Invalid input type ‘$type’; valid types: $types.")
              unless defined $c->stash->{inputTypes}->{$type};
...
sub latest_eval : Chained('jobsetChain') PathPart('latest-eval') {
    ...
    $c->res->redirect(
        $c->uri_for($c->controller('JobsetEval')->action_for("view"), [ $eval->id ], @args, $c->req->params));
}
