...
sub latestbuilds : Chained('api') PathPart('latestbuilds') Args(0) {
    ...
    my $filter = {finished => 1, fodchecker => 0};
...
sub queue : Chained('api') PathPart('queue') Args(0) {
    ...
    my @builds = $c->model('DB::Builds')->search({finished => 0, fodchecker => 0}, {rows => $nr, order_by => ["priority DESC", "id"]});
...
sub nrqueue : Chained('api') PathPart('nrqueue') Args(0) {
    my ($self, $c) = @_;
    my $nrQueuedBuilds = $c->model('DB::Builds')->search({finished => 0, fodchecker => 0})->count();
    $c->stash->{'plain'} = {
        data => "$nrQueuedBuilds"
    };
    $c->forward('Hydra::View::Plain');
