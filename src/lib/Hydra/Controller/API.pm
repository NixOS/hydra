...
sub push : Chained('api') PathPart('push') Args(0) {
    ...
    foreach my $r (@repos) {
        triggerJobset($self, $c, $_, $force) foreach $c->model('DB::Jobsets')->search(
            ...
            {
                join => 'project',
                where => \ [ 'exists (select 1 from JobsetInputs where project = me.project and jobset = me.name and value = ?)', [ 'value', $r ] ],
                order_by => 'me.id DESC'
            });
    }
...
sub push_github : Chained('api') PathPart('push-github') Args(0) {
    ...
    triggerJobset($self, $c, $_, 0) foreach $c->model('DB::Jobsets')->search(
        { 'project.enabled' => 1, 'me.enabled' => 1 },
        { join => 'project'
        , where => \ [ 'me.flake like ? or exists (select 1 from JobsetInputs where project = me.project and jobset = me.name and value like ?)', [ 'flake', "%github%$owner/$repo%"], [ 'value', "%github.com%$owner/$repo%" ] ]
        });
    $c->response->body("");
}
