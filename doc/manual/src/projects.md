...
### Valid keys for declarative jobsets

This is a list of keys supported by declarative jobset configurations.
Specifying unknown keys results in an error at evaluation time.

* **enabled** - whether the jobset should be enabled. Valid values are `true`,
    `false`, `oneshot`, `oneatatime`. Defaults to `true`.
* **hidden** - whether the jobset should be hidden. Valid values are `true`,
    and `false`.
* **description** - description of this project. Defaults to no description.
* **type** - whether the jobset is a legacy or flake-based jobset. Valid values
    are `legacy`, and `flake`. Defaults to `legacy`.
* **flake** - flake to build for flake-based projects. Required when `type` is
    `flake`.
* **nixexprinput** - name of the input where the declarative jobset
    configuration is found. Defaults to `declInput`.
* **nixexprpath** - path of a file from `nixexprinput` that is used for
    evaluation. Defaults to `default.nix`.
* **checkinterval** - interval in seconds to poll for updates. Defaults to
    `300`.
* **schedulingshares** - amount of scheduling shares to allocate for builds
    of the jobset. Defaults to `100`.
* **enableemail** - whether mails should be sent to maintainers when builds
    fail. Defaults to `false`.
* **emailoverride** - override for mails that would usually be sent to
    maintainers. Defaults to no override.
* **keepnr** - amount of evaluations to keep. Defaults to `3`.
* **inputs** - name-value pairs of inputs for the jobset. Defaults to an empty
    set.
