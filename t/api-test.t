...
subtest "login" => sub {
    subtest "bad password" => sub {
        my $result =
          request_json({ uri => "/login", method => "POST", data => { username => "root", password => "wrong" } });
        is($result->code(), 403, "Incorrect password rejected.");
    };

    my $result =
      request_json({ uri => "/login", method => "POST", data => { username => "root", password => "foobar" } });
    ...
    my $result = request_json(
        {
            uri    => '/project/sample',
            method => 'PUT',
            data   => { displayname => "Sample", enabled => "1", visible => "1", }
        }
    );
    ...
    my $result = request_json(
        {
            ...
            data   => {
                ...
                nixexprinput  => "my-src",
                inputs        => { "my-src" => { type => "path", value => $jobsetdir } },
                enabled       => "1",
                visible       => "1",
                checkinterval => "3600"
            }
...
subtest "evaluation" => sub {
    subtest "initial evaluaton" => sub {
        ok(evalSucceeds($db->resultset('Jobsets')->find({ name => 'default' })),
            "Evaluating should exit with return code 0");
    ...
    subtest "second evaluation" => sub {
        ...
        isnt(
            $evals->[0]->{jobsetevalinputs}->{"my-src"}->{revision},
            $evals->[1]->{jobsetevalinputs}->{"my-src"}->{revision},
            "Changing a jobset source changes its revision"
        );
        ...
        ok($build->{buildoutputs}->{out}->{path} =~ /\/nix\/store\/[a-zA-Z0-9]{32}-job$/,
            "The build's outpath is in the Nix store and named 'job'");
        ...
        subtest "search" => sub {
            ...
            my $search_build = decode_json(
                request_json({ uri => "/search/?query=" . $build->{buildoutputs}->{out}->{path} })->content());
            is(
                $search_build->{builds}[0]->{buildoutputs}->{out}->{path},
                $build->{buildoutputs}->{out}->{path},
                "Search for builds work"
            );
        };
    };
};
...
subtest "delete project" => sub {
    ...
    subtest "without evaluations and builds" => sub {
        my $project = request_json(
            {
                uri    => '/project/sample2',
                method => 'PUT',
                data   => { displayname => "Sample2", enabled => "1", visible => "1", }
            }
        );
        is($project->code(), 201, "PUTting a new project creates it");

        my $jobset = request_json(
            {
                uri    => '/jobset/sample2/default2',
                method => 'PUT',
                data   =>
                  { type => "1", flake => "github:nixos/nix", enabled => "1", visible => "1", checkinterval => "0" }
            }
        );
