...
subtest "dispatch_event" => sub {
    subtest "every plugin gets called once, even if it fails all of them." => sub {
        ...
        my @actual_names   = sort($event->{"called_with"}[0]->name, $event->{"called_with"}[1]->name);
        is(\@actual_names, \@expected_names, "Both plugins should be executed, but not in any particular order.");
    };
};
...
subtest "dispatch_task" => sub {
    subtest "every plugin gets called once" => sub {
        ...
        is($event->{"called_with"}[0]->name, "bogus-1", "Just bogus-1 should be executed.");
    };
    ...
    subtest "a failed run without a record saves the task for later" => sub {
        ...
        my $dispatcher       = Hydra::TaskDispatcher->new(
            $db,
            $prometheus,
            [$bogus_plugin],
            sub {
