...
subtest "isEnabled" => sub {
    is(Hydra::Plugin::RunCommand::isEnabled({}), "", "Disabled by default.");

    is(Hydra::Plugin::RunCommand::isEnabled({ config => {} }), "", "Disabled by default.");

    is(Hydra::Plugin::RunCommand::isEnabled({ config => { runcommand => {} } }),
        1, "Enabled if any runcommand blocks exist.");
};
...
subtest "configSectionMatches" => sub {
    subtest "Expected matches" => sub {
        ...
        for my $example (@examples) {
            ...
            is(Hydra::Plugin::RunCommand::configSectionMatches($matcher, $project, $jobset, $job),
                1, "Expecting $matcher to match $project:$jobset:$job");
        }
    };
    ...
    subtest "Fails to match" => sub {
        ...
        for my $example (@examples) {
            ...
            is(Hydra::Plugin::RunCommand::configSectionMatches($matcher, $project, $jobset, $job),
                0, "Expecting $matcher to not match $project:$jobset:$job");
        }

        like(
            dies {
                Hydra::Plugin::RunCommand::configSectionMatches("foo:bar:baz:tux", "foo", "bar", "baz"),
            },
...
subtest "eventMatches" => sub {
    ...
    # than buildFinished.
    is(Hydra::Plugin::RunCommand::eventMatches({}, "buildFinished"), 1, "An unspecified events key matches");

    is(Hydra::Plugin::RunCommand::eventMatches({ events => "" }, "buildFinished"),
        0, "An empty events key does not match");

    is(Hydra::Plugin::RunCommand::eventMatches({ events => "foo bar buildFinished baz" }, "buildFinished"),
        1, "An events key with multiple events does match when buildFinished is present");

    is(Hydra::Plugin::RunCommand::eventMatches({ events => "foo bar baz" }, "buildFinished"),
        0, "An events key with multiple events does not match when buildFinished is missing");
};
...
subtest "fanoutToCommands" => sub {
    ...
    is(
        Hydra::Plugin::RunCommand::fanoutToCommands($config, "buildFinished", "project", "jobset", "job"),
        [
            {
                matcher => "",
                command => "foo"
