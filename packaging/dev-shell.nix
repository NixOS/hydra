...
let
  ...
  internalDrvPaths = lib.listToAttrs (map (c: {
    name = builtins.unsafeDiscardStringContext c.drvPath;
    value = null;
  }) components);

  isInternal = dep:
    internalDrvPaths ? ${builtins.unsafeDiscardStringContext dep.drvPath or "_"};

  collectInputs = attr:
    lib.unique (
      lib.filter (x: !isInternal x) (
        lib.concatMap (c: c.${attr} or []) components
      )
    );
in

hydra.overrideAttrs (finalAttrs: prevAttrs: {
